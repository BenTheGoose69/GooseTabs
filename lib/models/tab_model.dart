...
class GuitarTab {
  ...
  String toTabFormat() {
    ...
    buffer.writeln('Tab Generated by GooseTabs: The Tab Writer - ${DateTime.now()}');
    ...
    for (final section in sections) {
      ...
      for (int stringIdx = 0; stringIdx < section.stringCount; stringIdx++) {
        final stringName = section.stringNames[stringIdx];
        ...
        for (int barIdx = 0; barIdx < section.bars.length; barIdx++) {
          ...
          for (int colIdx = 0; colIdx < bar.columns.length; colIdx++) {
            ...
            final note = column.notes[stringIdx];

            // Calculate max width for this column across all strings
            int maxLen = 1;
            for (int s = 0; s < section.stringCount; s++) {
              final n = column.notes[s];
              if (n.length > maxLen) maxLen = n.length;
            }

            // Pad note with dashes to match max width
            String paddedNote = note;
            if (note.length < maxLen) {
              paddedNote = note + ('-' * (maxLen - note.length));
            }
            buffer.write(paddedNote);

            // Add separator dash after each note for import compatibility
            buffer.write('-');
          }
          buffer.write('|');
        }
  ...
  static TabSection _parseSection(
    ...
    for (final barContent in barContents) {
      ...
      // Parse character by character, handling multi-char notes
      for (int pos = 0; pos < maxLen; pos++) {
        ...
        for (int stringIdx = 0; stringIdx < barContent.length; stringIdx++) {
          ...
          if (pos < content.length) {
            String note = content[pos];
            // Check for multi-digit fret numbers
            if (RegExp(r'\d').hasMatch(note)) {
              int endPos = pos + 1;
              while (endPos < content.length && RegExp(r'[\dh-]').hasMatch(content[endPos]) && content[endPos] != '-') {
                // Include technique modifiers like 'h' after number
                if (RegExp(r'[hpbr/\\~]').hasMatch(content[endPos])) {
                  note += content[endPos];
                  endPos++;
                  break;
                } else if (RegExp(r'\d').hasMatch(content[endPos])) {
                  note += content[endPos];
                  endPos++;
                } else {
                  break;
