...
class GuitarTab {
  ...
  factory GuitarTab.fromJson(Map<String, dynamic> json) => GuitarTab(
        ...
        songStructure: json['songStructure'] as String?,
      );
  ...
  String toTabFormat() {
    ...
    // Generate song structure if sections have labels
    final structureParts = <String>[];
    for (final section in sections) {
      if (section.label?.isNotEmpty == true) {
        if (section.repeatCount > 1) {
          structureParts.add('${section.label} x${section.repeatCount}');
        } else {
          structureParts.add(section.label!);
        }
      }
    }

    if (structureParts.isNotEmpty) {
      buffer.writeln();
      buffer.writeln('####################################');
      buffer.writeln('Song Structure');
      buffer.writeln(structureParts.join(' -> '));
      buffer.writeln('####################################');
    }
  ...
  /// Export with a custom song structure (allows reordering/duplicating sections)
  String toTabFormatWithStructure(String customStructure) {
    final buffer = StringBuffer();
    ...
    // Use the custom structure passed in
    if (customStructure.isNotEmpty) {
      buffer.writeln();
      buffer.writeln('####################################');
      buffer.writeln('Song Structure');
      buffer.writeln(customStructure);
      buffer.writeln('####################################');
    }
